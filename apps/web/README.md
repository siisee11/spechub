...
Implement prompts generated by the website tell a coding agent to download the full `specs/<slug>/` directory, including companion files alongside `SPEC.md`.
...
### Optional for implement-prompt generation in website build output

- `SPECHUB_REPO`: override GitHub owner/repo used in embedded installer commands (example: `openai/spechub`)
- `SPECHUB_REF`: override Git reference used in embedded installer commands (example: `main`)

...
const specRoot = join(repoRoot, "specs", "symphony-monitor");
...
test("symphony-monitor import contains the expected vendored file set", async () => {
  expect(await listFiles(specRoot)).toEqual(["SPEC.md", "UPSTREAM.md", "metadata.json"]);
});

test("symphony-monitor provenance metadata records upstream source", async () => {
  const metadata = await readFile(join(specRoot, "UPSTREAM.md"), "utf8");

  expect(metadata).toContain("https://github.com/siisee11/symphony-monitor.spec");
